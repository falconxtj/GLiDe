...
from sgRNA_design import NC_design
...
def get_genome_seq(prefix, genome_file, format):
    ...
    with open(genome_file, 'r') as file:
        with open('%s/map_view/result.fna' % prefix, 'a') as fna:
            ...
            name = lines[0][1:].split(' ')[0].split('|')[3]
            for line in lines:
                if line[0] == '>':
                    sequence_name = line[1:].split(' ')[0].split('|')[3]
...
def offtarget_filter(prefix, off_threshold, spacer_length, target_candidate_dic, nontarget_candidate_dic, position_dic,
    ...
    offtarget_log.to_excel('%sresult/offtarget_log.xlsx' % prefix, index=0)
...
def process_output(prefix, status):
    with open('%sresult/process_output' % prefix, 'w') as output_file:
        output_file.write(status)


def sentemail(receiver0, prefix):
    host = 'smtp.163.com'
    port = 465
    sender = 'crispri_fgpm@163.com'
    pwd = 'SILULPXHPHDCTNQE'
    body = '<p>Thank you for choosing crispri-fgpm.</p><p>Please find the attach files.</p><br><br><br><p>This email is generated by system.</p>'
    msg = MIMEText(body, 'html')
    message = MIMEMultipart()
    message['subject'] = 'CRISPRi FGPM sgRNA design result: request number ' + prefix
    message['from'] = sender
    message['to'] = receiver0
    message.attach(msg)

    offtarget_log = '%sresult/offtarget_log.xlsx' % prefix
    att1 = MIMEText(open(offtarget_log, 'rb').read(), 'base64', 'utf-8')
    att1["Content-Type"] = 'application/octet-stream'
    att1["Content-Disposition"] = 'attachment; filename=offtarget_log.xlsx'
    message.attach(att1)

    gene_info = '%sresult/gene_info.xlsx' % prefix
    att2 = MIMEText(open(gene_info, 'rb').read(), 'base64', 'utf-8')
    att2["Content-Type"] = 'application/octet-stream'
    att2["Content-Disposition"] = 'attachment; filename=gene_info.xlsx'
    message.attach(att2)
...
def generate_mapview(prefix):
    mission = prefix.split('/')[-1]
    fasta = '%s/map_view/result.fna' % prefix
    gff = '%s/map_view/result.gff3' % prefix
    gb = D3GB.genomebrowser((fasta,), directory='%s/map_view/result' % prefix)
    gb.addSequence(fasta)
    gb.addGFF(gff)
    # database file
    os.makedirs('/home/www/thu_big_homepage/static/db/map_view/%s' % mission)
    shutil.copy('%s/map_view/result/Tracks.db' % prefix, '/home/www/thu_big_homepage/static/db/map_view/%s/Tracks.db' % mission)
    # sequence file
    shutil.copytree('%s/map_view/result/sequences' % prefix, '/home/www/thu_big_homepage/static/sequence/map_view/%s' % mission)
    # JavaScript required
    with open('%s/map_view/result/scripts/query.js' % prefix, 'r') as js:
        lines = js.readlines()
        lines[3] = "d3.xhr('/static/db/map_view/%s/Tracks.db')\n" % mission
        lines[61] = "xhr.open('GET', '/static/sequence/map_view/%s/'+chr+'.fa', true);\n" % mission
    with open('%s/map_view/result/scripts/query.js' % prefix, 'w') as js:
        for line in lines:
            js.write(line)
    os.makedirs('/home/www/thu_big_homepage/static/script/map_view_scripts/%s' % mission)
    shutil.copy('%s/map_view/result/scripts/query.js' % prefix, '/home/www/thu_big_homepage/static/script/map_view_scripts/%s/query.js' % mission)
    # main index file
    with open('%s/map_view/result/index.html' % prefix, 'r') as index:
        lines = index.readlines()
        lines[6] = '<script type="text/javascript" src="/static/script/map_view_scripts/d3.min.js"></script>\n'
        lines[7] = '<script type="text/javascript" src="/static/script/map_view_scripts/jspdf.min.js"></script>\n'
        lines[8] = '<script type="text/javascript" src="/static/script/map_view_scripts/sql.js"></script>\n'
        lines[9] = '<script type="text/javascript" src="/static/script/map_view_scripts/functions.js"></script>\n'
        lines[10] = '<script type="text/javascript" src="/static/script/map_view_scripts/images.js"></script>\n'
        lines[11] = '<script type="text/javascript" src="/static/script/map_view_scripts/%s/query.js"></script>\n' % mission
        lines[12] = '<script type="text/javascript" src="/static/script/map_view_scripts/genomebrowser.js"></script>\n'
        lines[13] = '<script type="text/javascript" src="/static/script/map_view_scripts/genomemap.js"></script>\n'
    with open('%s/map_view/result/index.html' % prefix, 'w') as index:
        for line in lines:
            index.write(line)
    os.makedirs('/home/www/thu_big_homepage/templates/sgRNA_design/map_view/%s' % mission)
    shutil.copy('%s/map_view/result/index.html' % prefix, '/home/www/thu_big_homepage/templates/sgRNA_design/map_view/%s/index.html' % mission)


def main(prefix, configureFile):
    os.makedirs('%s/map_view' % prefix)
    process_output(prefix, '101')  # configure process
    starttime = datetime.datetime.now()
    var_dic = configure_process(configureFile)
    reference_file = var_dic['reference_file'].strip("['").strip("']").split("', '")
    ...
    process_output(prefix, '201')  # coding process and cluster identification
    ...
    gene_info = gene_info.reset_index(drop=True)
    fasta_info = pd.concat([fasta_info, sgRNA_info])
    fasta_info = fasta_info.reset_index(drop=True)
    fasta_info.to_csv('%s/map_view/result.gff3' % prefix, sep='\t', index=False, header=False)
    ...
    if NC_number > 0:
        ...
        gene_info.loc[len(gene_info.index)] = ['.', '.', 'NC RNA', '.', '.', '.', '.', '.', '.', '.', NC_list.tolist(),
                                               '.', NC_number]
    gene_info.to_excel('%sresult/gene_info.xlsx' % prefix, index=False)
    process_output(prefix, '601')

    try:
        with open('%s/email_output' % prefix, 'r') as file:
            email = file.readline()
        sentemail(email, prefix)
        process_output(prefix, '701')
    except FileNotFoundError:
        process_output(prefix, '701')

    generate_mapview(prefix)
